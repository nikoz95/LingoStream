"""Book routes: upload, list, detail, chapters, paragraphs, file serving, delete."""
import logging
import uuid
...
async def _get_owned_book(book_id: int, user_id: int, db: AsyncSession) -> Book:
    """Fetch a book and verify the requesting user owns it."""
...
async def _ensure_chapter_parsed(
    db: AsyncSession, chapter: Chapter, book_file_path: str,
) -> Chapter:
    """Lazy-parse a chapter: parse + persist paragraphs if not yet parsed."""
...
async def register_book(
    ...
    # Persist chapters
    chapter_entities = [
        Chapter(book_id=book.id, title=ch.get("title", f"Chapter {i + 1}"),
